...
namespace zorba
{
  namespace htmlmodule
  {
    ...
    static void checkOption(Bool ok)
    {
      if (ok != yes)
      {
        throw ZORBA_EXCEPTION( zerr::ZXQP0019_INTERNAL_ERROR, ERROR_PARAMS( "Could not set Tidy option" ) );
      }
    }
    ...
    static void checkRC(int rc, const char* errMsg)
    {
      if (rc > 1)
      {
        throw ZORBA_EXCEPTION( zerr::ZXQP0019_INTERNAL_ERROR, ERROR_PARAMS( errMsg ) );
      }
    }
